<script lang="ts">
    ...
    import { redirect, threadexport, layouturl, tabs, bookmarks, markhist, history } from "$lib/index.svelte";
...
<div class="flex justify-evenly items-center p-5">
    <div class="flex flex-col">
        <button type="button" class="btn preset-filled-secondary-500" onclick={opreply}>
            New Reply
        </button>
        {#if currentreply.layer>1}
        <button type="button" class="btn preset-filled-primary-500 mt-3" onclick={togglebacktrack}>
            {#if showtrail}
            Return
            {:else}
            Backtrack
...
{#if showtrail}
    <div class="flex w-full justify-center">
        <Thread boardname={params.boardname} threadid={params.thread} archive={false} threadata={threadata} bogo={false}></Thread>
    </div>
    {#each backtrack as reply}
        ...
        <div class="flex w-full justify-center">
            <Reply boardname={params.boardname} threadid={params.thread} replydata={reply} archive={false} bogo={false}></Reply>
        </div>
    {/each}
{:else}
    {#if currentreply.layer==1}
    ...
    {:else}
        <div class="flex w-full justify-center">
            <Reply boardname={params.boardname} threadid={params.thread} archive={false} replydata={previousreply} bogo={false}></Reply>
        </div>
    {/if}
{/if}
...
<div class="flex w-full justify-center">
    <Reply boardname={params.boardname} threadid={params.thread} archive={false} replydata={currentreply} bogo={false}></Reply>
</div>
